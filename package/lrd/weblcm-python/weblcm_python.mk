...
define WEBLCM_PYTHON_POST_INSTALL_TARGET_HOOK_CMDS
	...
	$(INSTALL) -D -t $(TARGET_DIR)/etc -m 644 $(WEBLCM_PYTHON_SITE)/weblcm-python.ini
	...
	sed -i -e '/\[weblcm\]/a enable_allow_unauthenticated_reboot_reset: \
	...
	sed -i -e '/\[weblcm\]/a allow_multiple_user_sessions: \
	...
	sed -i -e 's,^tools.sessions.on:.*,tools.sessions.on: \
		$(if $(findstring y,$(BR2_PACKAGE_WEBLCM_ENABLE_SESSIONS)),True,False),' $(TARGET_DIR)/etc/weblcm-python.ini

	sed -i -e '/^enable_client_auth/d' $(TARGET_DIR)/etc/weblcm-python.ini
	sed -i -e '/\[weblcm\]/a enable_client_auth: \
...
define WEBLCM_PYTHON_POST_INSTALL_TARGET_HOOK_CMDS3
	sed -i -e 's,^server.ssl_certificate_chain:.*,server.ssl_certificate_chain: \"$(WEBLCM_PYTHON_CA_CERT_CHAIN_PATH)\",' $(TARGET_DIR)/etc/weblcm-python.ini
endef
